...
function MenuGarage(currentSelection)
    ...
    if PlayerData.job.name == "police" then
        JobVehicle = Config.AuthorizedVehiclesLSPD[PlayerData.job.grade.level]
    elseif PlayerData.job.name == "sasp" then
        JobVehicle = Config.AuthorizedVehiclesSASP[PlayerData.job.grade.level]
    elseif PlayerData.job.name == "bcso" then
        JobVehicle = Config.AuthorizedVehiclesBCSO[PlayerData.job.grade.level]
    end


    for veh, label in pairs(JobVehicle) do
...
if Config.UseTarget then
    ...
    CreateThread(function ()
        ...
        while true do
            local sleep = 1000
            if inDuty and PlayerJob.name == "police" or PlayerJob.name == "bcso" or PlayerJob.name == "saspr" then
                sleep = 5
